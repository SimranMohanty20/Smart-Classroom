...
create_table_sql = f"CREATE TABLE IF NOT EXISTS {table_name} (name TEXT, qtime TEXT, date DATE, UNIQUE(name, date))"
...
def calculate_duration(start_time, end_time):
    ...
    diff = d2 - d1
    return diff.seconds / 60  # returns duration in minutes
...
class Face_Recognizer:
    ...
    def attendance(self, name):
        ...
        if existing_entry:  # If the name already exists in the database for the current date
            # Update the existing record with the new final_time
            start_time = existing_entry[1]  # Assuming the second column in your table is 'time'
            duration = calculate_duration(start_time, current_time)
            if duration > 55:  # If the duration is greater than 60 minutes
                cursor.execute("DELETE FROM attendance WHERE name=? AND date=?", (name, current_date))
                conn.commit()
                print(f"{name}'s record has been removed from the database.")
                # Insert a new record with the current time
                cursor.execute("INSERT INTO attendance (name, time, date, final_time) VALUES (?, ?, ?, ?)", (name, current_time, current_date, current_time))
                conn.commit()
                print(f"{name} marked as present for {current_date} at {current_time}")
            else:
                 # Update the existing record
                cursor.execute("UPDATE attendance SET final_time = ? WHERE name = ? AND date = ?", (current_time, name, current_date))
                print(f"{name}'s record has been updated for {current_date} at {current_time}")
